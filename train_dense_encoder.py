...
class BiEncoderTrainer(object):
    ...
    def run_train(self, ):
        ...
        process_fn = BiEncoder.get_input_create_fn(
            self.tensorizer, False, args.hard_negatives, args.other_negatives,  # insert_title = False
            shuffle=True,
    ...
    def validate_average_rank(self) -> float:
        ...
        averaged rank：每个问题的相关段落的位置的和 / 问题总数 该值越小越好！
        """
        ...
        sub_batch_size = args.val_av_rank_bsz  # 128
        sim_score_f = BiEncoderNllLoss.get_similarity_function()
        q_represenations = []
        ctx_represenations = []
        positive_idx_per_question = []

        num_hard_negatives = args.val_av_rank_hard_neg  # 30
        num_other_negatives = args.val_av_rank_other_neg  # 30 # dev文件没有提供negatives

        log_result_step = args.log_batch_step

        for i, samples_batch in enumerate(data_iterator.iterate_data()):
            # samples_batch size = 16
            # samples += 1
            ...
            ctxs_ids = biencoder_input.context_ids  # 即ctxs_tensors
            ctxs_segments = biencoder_input.ctx_segments  # 全0
            bsz = ctxs_ids.size(0)  # 16（因为dev没有提供negatives, 所以只有16条段落）
            ...
            for j, batch_start in enumerate(range(0, bsz, sub_batch_size)):
                ...
                if q_dense is not None:
                    q_represenations.extend(q_dense.cpu().split(1, dim=0))  # 拉直操作
