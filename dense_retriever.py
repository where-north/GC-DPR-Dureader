        ...
        with torch.no_grad():
            for j, batch_start in enumerate(range(0, n, bsz)):
                ...
                for idx, item in enumerate(out):
                    if torch.isnan(item).any().item() is True:
                        print(batch_token_tensors[idx])
                        raise RuntimeError('错误：问题编码出现 nan 值！')
...
def parse_dureader_json_file(location) -> Iterator[Tuple[str, str]]:
    with open(location) as ifile:
...
def load_passages(ctx_file: str, with_title=True) -> Dict[object, Tuple[str, str]]:
    docs = {}
...
def save_dureader_results(question_ids: List[str],
   ...
   "5a279eb92eb911b1f46c3ea4894a0df2":[
      ...
      "cd76c3d844db75ae7718618453197427",
      "...",
      "fa33a37a8ac6c19c904cea2feb824be6",
      "5c48479246cecc3c966c0a8e1c0bd481"
   ],}
   data4rerank:
   [
   {'q_text': '',
...
def main(args):
    ...
    if args.dureader_test:
        ...
        questions = []
        question_ids = []
        for ds_item in parse_dureader_json_file(args.qa_file):
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dureader_test", action='store_true')
